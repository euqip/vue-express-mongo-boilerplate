"use strict"

let logger 		= require("../../../core/logger")
let config 		= require("../../../config")
let C 	 		= require("../../../core/constants")

//let _			= require("lodash")

let User 		= require("../../../models/user")
let modulename = "users"

module.exports = {
  settings: {
    name: "users",
    version: 1,
    namespace: "users",
    rest: true,
    ws: true,
    graphql: true,
    permission: C.PERM_ADMIN,
    role: "user",
    collection: User,
    modelPropFilter: "id code username fullName avatar lastLogin roles locale email passwordLess verified"
  },

  actions: {
    find: {
      cache: true,
      handler(ctx) {
        let filter = {}

        let query = User.find(filter)
        return ctx.queryPageSort(query).exec().then( (docs) => {
          return this.toJSON(docs)
        })
      }
    },

    // return a model by ID
    get: {
      cache: true,
      handler(ctx) {
        ctx.assertModelIsExist(ctx.t(modulename +":NotFound"))
        return Promise.resolve(ctx.model)
      }
    },

    create(ctx) {
      this.validateParams(ctx, true)

      let item = new User({
        username: ctx.params.username,
        email: ctx.params.email,
        fullname: ctx.params.fullname,
        passwordLess: ctx.params.passwordLess,
        verified: ctx.params.verified
      })

      return item.save()
      .then((doc) => {
        return this.toJSON(doc)
      })
      .then((json) => {
        return this.populateModels(json)
      })
      .then((json) => {
        this.notifyModelChanges(ctx, "app:created", json)
        return json
      })
    },

    update(ctx) {
      ctx.assertModelIsExist(ctx.t(modulename + ":NotFound"))
      this.validateParams(ctx)

      return this.collection.findById(ctx.modelID).exec()
      .then((doc) => {

        if (ctx.params.roles != null)
          doc.roles = ctx.params.roles

        if (ctx.params.fullname != null)
          doc.fullname = ctx.params.fullname

        if (ctx.params.avatar != null)
          doc.avatar = ctx.params.avatar

        if (ctx.params.verified != null)
          doc.verified = ctx.params.verified

        if (ctx.params.passwordLess != null)
          doc.passwordLess = ctx.params.passwordLess

        return doc.save()
      })
      .then((doc) => {
        return this.toJSON(doc)
      })
      .then((json) => {
        return this.populateModels(json)
      })
      .then((json) => {
        this.notifyModelChanges(ctx, "app:updated", json)
        return json
      })
    },

    remove(ctx) {
      ctx.assertModelIsExist(ctx.t(modulename + ":NotFound"))

      return User.remove({ _id: ctx.modelID })
      .then(() => {
        return ctx.model
      })
      .then((json) => {
        this.notifyModelChanges(ctx, "app:removed", json)
        return json
      })
    }

  },

  methods: {
    /**
     * Validate params of context.
     * We will call it in `create` and `update` actions
     *
     * @param {Context} ctx 			context of request
     * @param {boolean} strictMode 		strictMode. If true, need to exists the required parameters
     */
    validateParams(ctx, strictMode) {
      if (strictMode || ctx.hasParam("username"))
        ctx.validateParam("username").trim().notEmpty(ctx.t(modulename + ":userNameCannotBeBlank")).end()

      if (strictMode || ctx.hasParam("status"))
        ctx.validateParam("status").isNumber()

      ctx.validateParam("description").trim().end()
      ctx.validateParam("address").trim().end()
      ctx.validateParam("type").trim().end()

      if (ctx.hasValidationErrors())
        throw ctx.errorBadRequest(C.ERR_VALIDATION_ERROR, ctx.validationErrors)
    }
  },

  init(ctx) {
    // Fired when start the service
  },

  socket: {
    afterConnection(socket, io) {
      // Fired when a new client connected via websocket
    }
  },

  graphql: {

    query: `
      devices(limit: Int, offset: Int, sort: String): [User]
      device(code: String): User
    `,

    types: `
      type User {
        code: String!
        address: String
        type: String
        name: String
        description: String
        status: Int
        lastCommunication: Timestamp
      }
    `,

    mutation: `
      deviceCreate(name: String!, address: String, type: String, description: String, status: Int): Device
      deviceUpdate(code: String!, name: String, address: String, type: String, description: String, status: Int): Device
      deviceRemove(code: String!): User
    `,

    resolvers: {
      Query: {
        devices: "find",
        device: "get"
      },

      Mutation: {
        deviceCreate: "create",
        deviceUpdate: "update",
        deviceRemove: "remove"
      }
    }
  }

}

/*
## GraphiQL test ##

# Find all devices
query getDevices {
  devices(sort: "lastCommunication", limit: 5) {
    ...deviceFields
  }
}

# Create a new device
mutation createDevice {
  deviceCreate(name: "New device", address: "192.168.0.1", type: "raspberry", description: "My device", status: 1) {
    ...deviceFields
  }
}

# Get a device
query getDevice($code: String!) {
  device(code: $code) {
    ...deviceFields
  }
}

# Update an existing device
mutation updateDevice($code: String!) {
  deviceUpdate(code: $code, address: "127.0.0.1") {
    ...deviceFields
  }
}

# Remove a device
mutation removeDevice($code: String!) {
  deviceRemove(code: $code) {
    ...deviceFields
  }
}

fragment deviceFields on Device {
    code
    address
    type
    name
    description
    status
    lastCommunication
}

*/